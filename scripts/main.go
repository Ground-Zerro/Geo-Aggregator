...
func main() {
	...
	catalogCategories, catalogServices := buildCatalog(index, now)

	fmt.Println("=== Генерация README.md ===")
	writeREADME(repo, branch, len(index), catalogCategories, catalogServices, now)
...
func writeREADME(repo, branch string, count, catalogCategories, catalogServices int, now time.Time) {
	lines := []string{
		"# Geo-Aggregator",
		"",
	...
	if catalogServices > 0 {
		lines = append(lines,
			"Каталог сервисов — категории и входящие в них теги:",
			"```",
			fmt.Sprintf("https://raw.githubusercontent.com/%s/%s/db/catalog.json", repo, branch),
			fmt.Sprintf("https://raw.githubusercontent.com/%s/%s/db/catalog.json.gz", repo, branch),
			"```",
			"",
			fmt.Sprintf("%d категорий, %d сервисов. Состав задаёт `db/catalog.map.yml`; ссылка на список тега собирается из полей `base` и `src`.",
